...
namespace Project
{
    public class AICharacterMotor : CharacterMotor
    {
        ...
        public void SteerTowards(Vector3 target, float rotationSpeed, float stoppingDistance, float speed)
        {
            ...
            Vector3 targetVelocity = Vector3.Lerp(_state.LinkedRB.velocity, desiredVelocity, _desiredVelocityWeighting);
            ...
            _state.Input_Move.y = movementScale * Mathf.Clamp(Vector3.Dot(targetVelocity, transform.forward) / _movementMode.CurrentMaxSpeed, -1f, 1f);
            _state.Input_Move.x = movementScale * Mathf.Clamp(Vector3.Dot(targetVelocity, transform.right) / _movementMode.CurrentMaxSpeed, -1f, 1f);
        }

        public void Stop()
        {
            _state.Input_Move = Vector2.zero;
        }
