...
namespace Project
{
    ...
    public class CharacterMotor : NetworkBehaviour, IDamageable
    {
        ...
        protected Rigidbody _linkedRB;
        protected CapsuleCollider _linkedCollider;
        protected ClientNetworkTransform _clientNetworkTransform;
        protected GravityTracker _localGravity;
        ...
        public Vector3 UpVector => _localGravity != null ? _localGravity.Up : Vector3.up;
        public Vector3 DownVector => _localGravity != null ? _localGravity.Down : Vector3.down;
        ...
        {          
            ...
            _linkedCollider = GetComponent<CapsuleCollider>();
            _clientNetworkTransform = GetComponent<ClientNetworkTransform>();
            _localGravity = GetComponent<GravityTracker>();
        ...
        protected void Move()
        {
            if(_localGravity == null)
                _linkedRB.AddForce(Physics.gravity, ForceMode.Acceleration);
        ...
        protected RaycastHit UpdateIsGrounded()
        {
            ...
            // get bottom of RB
            Vector3 startPos = _linkedRB.position + UpVector * CurrentHeight * 0.5f;
            ...
            if (Physics.Raycast(startPos, DownVector, out groundHitResult,
        ...
        protected void UpdateMovement(Vector2 inputVector)
        {
            ...
            if(IsGroundedOrInCoyoteTime)
            {
                ...
                if(movementVector.y > 0 && Vector3.Angle(UpVector, _groundedHitResult.normal) > _config.SlopeLimit)
                    movementVector = Vector3.zero;              
            } // in the air
            else
                movementVector += DownVector * _config.FallVelocity;
        ...
        {
            Vector3 lookAheadStartPoint = transform.position + UpVector * (_config.StepCheck_MaxStepHeight * 0.5f);
            ...
            {
                lookAheadStartPoint = transform.position + UpVector * _config.StepCheck_MaxStepHeight;
                                    ...
                                    _config.GroundedLayerMask, QueryTriggerInteraction.Ignore))
                {
                    ...
                    if(Physics.Raycast(candidatePoint, DownVector, out hitResult, _config.StepCheck_MaxStepHeight * 2f, 
                                       _config.GroundedLayerMask, QueryTriggerInteraction.Ignore))
                    {
                        // is the step shallow enough in slope
                        if(Vector3.Angle(UpVector, hitResult.normal) <= _config.SlopeLimit)
                        {
